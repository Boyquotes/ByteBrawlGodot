...
const projectile_lifetime_min: float = 1.
const projectile_lifetime_max: float = 10.
var projectile_lifetime: float = 2.:
	set(x): projectile_lifetime = clamp(x, projectile_lifetime_min, projectile_lifetime_max)
...
func activate():
	if _owner.target_locator:
		...
		materia_projectile.position = _owner.position
...
func init_fields():
	...
	fields.append_array([
		...
		Field.Float(
			"projectile_lifetime",
			"Projectile Lifetime",
			func(): return projectile_lifetime,
			func(x): projectile_lifetime = x,
			projectile_lifetime_min,
...
func _to_string() -> String:
	return """MateriaProjectile | projectile_life_time : %.2f""" % projectile_lifetime
